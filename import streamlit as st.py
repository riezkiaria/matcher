            ...
            if inv_num in invoice_index and invoice_index[inv_num] not in matched_invoice_ids:
                ...
                results.append({
                    **bank_row.to_dict(),
                    ...
                    'confidence': 'HIGH',
                    'match_type': 'EXACT_INVOICE',
                    'reasons': '✅ EXACT INVOICE DITEMUKAN DI DESKRIPSI',
                    'Matching_Note': 'Exact Invoice Match',
                    'is_combination': False,
                    'matched_invoice_count': 1,
                    'customer_similarity': 100,
...
def generate_excel_output(df_result):
    ...
    display_columns = [
        ...
        'score', 'Saldo Piutang', 'amount_diff', 'Reference No.', 'bank_id'
    ]
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        format_sheet(ws_summary, summary_df, has_confidence_color=False)

        # Format amount di summary
        for row in range(2, 7):
            ws_summary.cell(row=row, column=3).number_format = 'Rp #,##0.00'

        # Warnai baris di summary
        ws_summary.cell(row=2, column=1).fill = high_fill
        ws_summary.cell(row=3, column=1).fill = medium_fill
        ws_summary.cell(row=4, column=1).fill = low_fill
        ws_summary.cell(row=5, column=1).fill = none_fill
    ...
    if bank_file and aging_file:
        if st.button("🚀 Start Advanced Matching", type="primary", width='stretch') or 'df_result' in st.session_state:
            ...
            st.download_button(
                ...
                data=excel_file,
                file_name="advanced_reconciliation_result.xlsx",
